...
// Łódź: ONE feed on the sheet — the ZDiT bundle from the city's open-data
// portal (otwarte.miasto.lodz.pl, produced by R&G PLUS), split by route_type
// into buses (3) and trams (0). No metro and no suburban-rail slot here: the
// tram network is the whole rail side, the two regional lines included (41 to
// Pabianice, 45 to Zgierz), and the sheet reaches as far as the ZDiT county
// buses run — Zgierz and Stryków in the north, Pabianice and Rzgów in the
// south, Aleksandrów, Konstantynów and Lutomiersk in the west, Brzeziny and
// Andrespol in the east (40 × 30 km all told).
// Line keys are the operator's OWN designations, unchanged — including the
// bus "6." with its trailing dot, which is how MPK's own timetables write the
// Łódź Kaliska – Zgierz zone line (mpk.lodz.pl, line id 1127).
...
// ONE feed, TWO cfgs: routeTypes splits the ZDiT bundle per mode (3 = bus,
// 0 = tram). Without the filter `--all` on the bus mode would swallow the
// trams too.
//
// skipRoute drops what rides in the timetable but is not a passenger line:
//   • the R lines (tram R8, buses R9…R26) — "linie zjazdowe", the depot
//     pull-ins of the line whose number they carry. R8's entire timetable is
//     two trips between the same pole at the Telefoniczna depot.
//   • P1/P2 (trams), P4/P7 and oP6 (buses) — "przewóz pracowników", MPK's own
//     staff shuttles: that phrase is the headsign of every single trip.
// The Z lines stay. Unlike Warsaw's Z-buses or Budapest's pótló they are not a
...
// No rail trunk in Łódź: nothing gets the metro treatment (wide ribbon,
// station discs, always-on names) — the regional trams 41 and 45 are trams.
const isRailTrunk = () => false;
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/lodz-rail.json',
