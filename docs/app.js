// Frontend: MapLibre GL + OpenFreeMap vector tiles (positron) + ZDiT line layers
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS ZDiT Łódź (otwarte.miasto.lodz.pl) · MZK Pabianice · MUK Zgierz (rozklady.miasto.zgierz.pl)' }));
  ...
  // Three operators share this sheet and they share numbers: Pabianice's 1–7,
  // Zgierz's 1–10 and 61, ZDiT's own trams 1–16 and bus 61. Each chip prints
  // what its own stop flag prints (meta carries the label where the key had to
  // take an operator prefix), so the list is grouped by operator — the Berlin
  // panel. Inside an operator, buses and trams stay apart.
  const OPS = Object.entries(meta.ops || { zdit: 'ZDiT — Łódź' });
  const OP_TITLE = new Map(OPS);
  const CATS = [['bus', 'Buses'], ['tram', 'Trams']];
  ...
  const paintChips = (linesView) => {
    ...
    const section = (key) => {
      ...
      const groups = CATS.map(([c, title]) => [title, ls.filter((l) => l.mode === c)])
        .filter(([, cl]) => cl.length);
      const many = bucket.size || OPS.length > 1;
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: ZDiT Łódź · MZK Pabianice · MUK Zgierz';
      ...
      a.download = `lodz-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `lodz-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  function wireJourneyPlanner() {
    ...
    const applyJourney = (opt) => {
      ...
      for (const leg of opt.legs) {
        ...
        feats.push({ type: 'Feature', properties: { kind: 'leg', color: leg.color, line: lineLabel(leg.line) }, geometry: { type: 'LineString', coordinates: line } });
    ...
    const esc2 = esc;
    const lineLabel = (k) => { const l = meta.lines.find((x) => x.line === k); return (l && l.label) || k; };
    const renderResults = (opts) => {
      resEl.innerHTML = '';
      opts.forEach((o, i) => {
        const li = document.createElement('li');
        const parts = o.legs.map((l, li) => {
          const all = (li === 0 && o.alt1 ? [l.line, ...o.alt1] : [l.line]).map(lineLabel);
  ...
  // NOT fitBounds(meta.bbox): the county buses run out to Stryków, Brzeziny
  // and Rzgów and the data bbox is 40 × 30 km, so fitting it would open on
  // fields. Open on the city, the inner suburbs (Zgierz, Pabianice,
  // Aleksandrów, Konstantynów) inside the frame.
  map.jumpTo({ center: [19.46, 51.775], zoom: 10.9 });
}
